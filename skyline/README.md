...
| `scripts/` | Study drivers (`sky_dual_study.py`, `sky_dual_hardcity.py`, `sky_matcher_resolution.py`, `sky_recog_radius.py`) and `silver_infer.py`, the SegFormer inference script. |
...
this directory on `sys.path`, so no installation step is needed:
